...
typedef float movement_t[{motors_per_movement}];\n
"""
...
for name, trajectory in precomputed_gaits.items():
    ...
    header += f"\n\t// {name}\n"
    for movements in trajectory:
        header += f"\t{{ {", ".join([str(float(x)) for x in movements])} }},\n"
        curr_offset += 1

header += "};\n\n"

header += "const static movement_t GaitIdleLUT[] = {\n"

for dir in GAIT_DIRECTIONS:
    for k, v in GAITS.items():
        q1, q2, success = q1, q2, success = leg.ik_solve(v[1], v[2], True, 1)
        header += f"\t// {k}_{dir}\n"
        header += f"\t{{ {', '.join([str(q1), str(q2)] * 4)} }},\n"

header += "};\n\n"

# Generate offsets so we know how many movements make up each gait. Indexed by enum values.
header += "const static uint32_t GaitLUTOffsets[] = {\n"

for offset in gait_offsets.values():
    header += f"\t{offset},\n"

header += "\tsizeof(GaitLUT),\n"
